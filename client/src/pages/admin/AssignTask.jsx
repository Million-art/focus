  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        // Fetch admins
        const adminResponse = await axios.get(`${apiBaseUrl}/employee/employees`);
        const adminEmployees = adminResponse.data.filter((employee) => employee.position === 'Admin');
    ...
    if (newTask.name !== '' && newTask.description !== '' && newTask.assign_to !== '' && newTask.task_type !== '') {
      try {
        ...
        if (response.status === 201) {
          toast.success(' task assigned successfully', { position: toast.POSITION.TOP_RIGHT });
          ...
          toast.warning('Error creating task', { position: toast.POSITION.TOP_RIGHT });
        }
      } catch (error) {
        // Handle network error
        toast.error('Error creating task', { position: toast.POSITION.TOP_RIGHT });
      }
    } else {
