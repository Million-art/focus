...
import RemotEmployeesNav from '../../../components/RemotEmployeesNav';
...
const RemotEmployeesHome = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get(`${apiBaseUrl}/tasks/task`, { params: { userEmail } });
  ...
  const handleStartTask = async (taskId) => {
    try {
      if (inProgressTaskId) {
        ...
        console.log('start', start_time, taskId);
        return;
      }
